define(['kendo', 'odemeDurumuDatasource', 'router'],
function (kendo, odemeDurumuDatasource, router) {
    var lastSelectedDataItem = null;
    var onClick = function (event, delegate) {
        event.preventDefault();
        var grid = $("#durumgrid").data("kendoGrid");
    ...
    function newRecord(e) {
        e.preventDefault();
        router.navigate('/DurumTanimlari/Edit/0');
    };
    var indexViewModel = new kendo.data.ObservableObject({
        ...
        details: function (event) {
            onClick(event, function (grid, row, dataItem) {
                router.navigate('/DurumTanimlari/Edit/' + dataItem.Id);
            });
        },
        edit: function (event) {
            onClick(event, function (grid, row, dataItem) {
        ...
        destroy: function (event) {
            onClick(event, function (grid, row, dataItem) {
                ...
                if (r == true) {
                    odemeDurumuDatasource.remove(dataItem);
        ...
        dataSource: odemeDurumuDatasource,
