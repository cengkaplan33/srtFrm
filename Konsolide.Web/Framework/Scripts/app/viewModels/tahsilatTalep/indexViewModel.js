define(['kendo', 'odemeTalepModel', 'tahsilatTalepDatasource', 'tahsilatTuruDatasource', 'TahsilatEkraniDurumTanimiDatasource', 'router'],
function (kendo, odemeTalepModel, tahsilatTalepDatasource, tahsilatTuruDatasource, TahsilatEkraniDurumTanimiDatasource, router) {
    ...
    //     var grid = $("#tahsilatTalepGrid").data("kendoGrid");
    ...
    var toolbar;
    function LoadGrid() {
        grid = $("#tahsilatTalepGrid").kendoGrid({
            dataSource: tahsilatTalepDatasource,
            groupable: false,
            sortable: true,
            scrollable: false,
            editable: "inline",
        ...
        $("#tahsilatTalepGrid").on("click", "td", function (e) {

            var rowIndex = $(this).parent().index();
            var cellIndex = $(this).index();
            $("input").on("keydown", function (event) {
                if (event.keyCode == 13) {
                    $("#tahsilatTalepGrid").data("kendoGrid").editCell($(".k-grid-content").find("table").find("tbody").find("tr:eq(" + rowIndex + ")").find("td:eq(" + cellIndex + ")").next().focusin($("#batchgrid").data("kendoGrid").closeCell($(".k-grid-content").find("table").find("tbody").find("tr:eq(" + rowIndex + ")").find("td:eq(" + cellIndex + ")").parent())));
                    return false;
                }
            });
    ...
    function odemeTuruEditor(container, options) {
        ...
        ddEditor.appendTo(container).kendoDropDownList({
            dataSource: tahsilatTuruDatasource,
            dataTextField: "Name",
            dataValueField: "Id"
        });
    ...
    function odemeDurumuEditor(container, options) {
        ...
        ddEditor.appendTo(container).kendoDropDownList({
            dataSource: TahsilatEkraniDurumTanimiDatasource,
            dataTextField: "Durum",
            dataValueField: "Id"
        });
    };
